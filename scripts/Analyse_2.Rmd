...
data_P2 <- data_P2 %>%
  mutate(
    lieu_de_naissance = ifelse(lieu_de_naissance == "Hors du Canada", 
                               "Hors du Quebec", 
                               lieu_de_naissance),
    
    lieu_de_naissance_mere = ifelse(lieu_de_naissance_mere == "Hors du Canada",
                                    "Hors du Québec",
                                    lieu_de_naissance_mere),
    
    lieu_de_naissance_pere = ifelse(lieu_de_naissance_pere == "Hors du Canada",
                                    "Hors du Québec",
                                    lieu_de_naissance_pere),
    
    lieu_de_naissance_les_deux = ifelse(lieu_de_naissance_les_deux == "Hors du Canada",
                                        "Hors du Québec",
                                        lieu_de_naissance_les_deux)
  )
...
data_P2 <- data_P2 %>%
  mutate(Occupation_du_repondant = case_when(
    Occupation_du_repondant %in% c(
      "À la maison TPL",
      "Sans travail mais recherche",
      "Sans travail sans recherche"
    ) ~ "Sans travail",
    ...
    Occupation_du_repondant %in% c(
      "Travail et études TPA",
      "Travail PTL études TPA",
      "Travail TPA études PTL"
    ) ~ "Travail et études",
    ...
    TRUE ~ Occupation_du_repondant
  ))


#Revenu_annuel_du_repondant
data_P2 <- data_P2 %>%
  mutate(Revenu_annuel_du_repondant = case_when(
    Revenu_annuel_du_repondant %in% c("Ne sait pas", "Préfère ne pas répondre") ~ "Ne sait pas",
    TRUE ~ Revenu_annuel_du_repondant
  ))
...
data_P2 <- data_P2 %>%
  mutate(
    Occupation_mere = case_when(
      Occupation_mere %in% c(
        "Agriculeurs exploitants",
        "Artisants, commerçants, chef",
        "Ouvriers"
      ) ~ "Travailleurs",
      ...
      TRUE ~ Occupation_mere
    ),
    
    Occupation_pere = case_when(
      Occupation_pere %in% c(
        "Agriculeurs exploitants",
        "Artisants, commerçants, chef",
        "Ouvriers"
      ) ~ "Travailleurs",
...
data_P2 <- data_P2 %>%
  mutate(Siruation_civile_parents = case_when(
    Siruation_civile_parents %in% c(
      "Les deux sont defunts",
      "Un des deux est defunt"
    ) ~ "Au moins un des deux est défunts",
    
    TRUE ~ Siruation_civile_parents
  ))
...
#
data_P2 <- data_P2 %>%
  mutate(Etat_civil_du_repondant = case_when(
    Etat_civil_du_repondant %in% c(
    ...
    Etat_civil_du_repondant %in% c(
      "Conjoint de fait avec enfant",
      "Marié avec enfant"
    ) ~ "Avec conjoint et enfant",
    
    Etat_civil_du_repondant %in% c(
      "Conjoint de fait sans enfant",
      "Marié sans enfant"
    ) ~ "Avec conjoint sans enfant",
    
    TRUE ~ Etat_civil_du_repondant
  ))



data_P2 <- data_P2 %>%
  mutate(Type_de_domicile_mere = case_when(
    Type_de_domicile_mere %in% c(
      "Propriétaire condo",
      "Propriétaire maison"
    ) ~ "Propriétaire",
    
    TRUE ~ Type_de_domicile_mere
  ))
...
data_P2 <- data_P2 %>%
  mutate(Type_de_domicile_pere = case_when(
    Type_de_domicile_pere %in% c("Propriétaire condo", "Propriétaire maison") ~ "Propriétaire",
    Type_de_domicile_pere %in% c("En RAP", "Incaecere") ~ "Autres",
    TRUE ~ Type_de_domicile_pere
  ))


#table(data_P2$lieu_de_naissance)
```
