...
data_P2 <- data_P2 %>%
  mutate(
    Votre_mere = factor(Votre_mere, levels = 1:5, labels = c("Pas important du tout", "Pas important", "Plus ou moins important", "Important", "Très important")),
    Votre_pere = factor(Votre_pere, levels = 1:5, labels = c("Pas important du tout", "Pas important", "Plus ou moins important", "Important", "Très important")),
    Votre_soeur = factor(Votre_soeur, levels = 1:5, labels = c("Pas important du tout", "Pas important", "Plus ou moins important", "Important", "Très important")),
    Votre_frere = factor(Votre_frere, levels = 1:5, labels = c("Pas important du tout", "Pas important", "Plus ou moins important", "Important", "Très important")),
    Vos_amis = factor(Vos_amis, levels = 1:5, labels = c("Pas important du tout", "Pas important", "Plus ou moins important", "Important", "Très important")),
    A_l_ecole = factor(A_l_ecole, levels = 1:5, labels = c("Pas important du tout", "Pas important", "Plus ou moins important", "Important", "Très important")),
    Vos_colleges_de_travail = factor(Vos_colleges_de_travail, levels = 1:5, labels = c("Pas important du tout", "Pas important", "Plus ou moins important", "Important", "Très important")),
    Les_medias_traditionnels = factor(Les_medias_traditionnels, levels = 1:5, labels = c("Pas important du tout", "Pas important", "Plus ou moins important", "Important", "Très important")),
    Les_medias_numeriques_Q1 = factor(Les_medias_numeriques_Q1, levels = 1:5, labels = c("Pas important du tout", "Pas important", "Plus ou moins important", "Important", "Très important"))
  )
...
data_P2 <- data_P2 %>%
  mutate(
    Propres_responsabilites = factor(Importance_propres_responsabilites, levels = 1:5, labels = c("Pas important du tout", "Pas important", "Plus ou moins important", "Important", "Très important")),
    Propres_decisions = factor(Importance_propres_decisions, levels = 1:5, labels = c("Pas important du tout", "Pas important", "Plus ou moins important", "Important", "Très important")),
    Independance_finance = factor(Importance_independant_finance, levels = 1:5, labels = c("Pas important du tout", "Pas important", "Plus ou moins important", "Important", "Très important"))
  )
...
# Variables indépendantes par modèle
indep_vars_1a <- c("Usage_MS")  # Modèle 1a
indep_vars_1b <- c("Nbr_compte_MS")  # Modèle 1b
indep_vars_1c <- c("Nbr_compte_MS_regulier")  # Modèle 1c
...
#3. Boucle pour exécuter tous les modèles et stocker les résultats
```{r message=FALSE, warning=FALSE}

# Filtrer les données pour ne garder que les personnes ayant répondu "Oui" à Usage_MS


# Lancer toutes les régressions et stocker les résultats
results <- map_dfr(dependent_vars, function(dep_var) {
  bind_rows(
    ...
    run_logistic_regression(dep_var, indep_vars_1d, "Modèle 1d", data_P2),
...
ggplot(results, aes(x = OR, y = Variable, xmin = CI_low, xmax = CI_high, color = Model)) +
  ...
  scale_x_continuous(trans = 'log') +  # Transformation de l'axe x en échelle logarithmique
  labs(x = "Odds Ratio (OR)", y = "Variable", title = "Forest Plot des régressions logistiques") +
  theme_minimal() +
...
# Création du forest plot avec les variables indépendantes en colonnes et les variables dépendantes en lignes
gr=ggplot(results, aes(x = OR, y = Dependent, xmin = CI_low, xmax = CI_high, color = Variable)) +
  geom_point(size = 3) +  # Points pour les OR
  geom_errorbarh(height = 0.2) +  # Barres horizontales pour les intervalles de confiance
  scale_x_continuous(trans = 'log') +  # Transformation de l'axe x en échelle logarithmique
  facet_wrap(~ Model, scales = "free_y", ncol = length(indep_vars_1d)) +  # Facet par modèle avec les indépendantes en colonne
  labs(x = "Odds Ratio (OR)", y = "Variable Dépendante", title = "Forest Plot des régressions logistiques") +
  theme_minimal() +
  theme(axis.text.y = element_text(size = 8),  # Taille des labels y
...
# Création du forest plot avec des résultats significatifs mis en évidence
z=ggplot(results, aes(x = OR, y = Dependent, xmin = CI_low, xmax = CI_high, color = Significant)) +
  geom_point(size = 3) +  # Points pour les OR
  geom_errorbarh(height = 0.2) +  # Barres horizontales pour les intervalles de confiance
  scale_x_continuous(trans = 'log', breaks = c(0.5, 1, 2, 5, 10)) +  # Transformation de l'axe x en échelle logarithmique
  geom_vline(xintercept = 1, linetype = "dashed", color = "black") +  # Ligne verticale pour OR = 1
  facet_wrap(~ Model, scales = "free_y", ncol = 2) +  # Facet par modèle avec les indépendantes en colonne
  labs(x = "Odds Ratio (OR)", y = "Variable Dépendante", title = "Forest Plot des régressions logistiques") +
  theme_minimal() +
  theme(axis.text.y = element_text(size = 8),  # Taille des labels y
        legend.position = "bottom")  # Position de la légende
...
models <- lapply(dependent_vars, function(dep_var) {
  list(
    "Modèle 1a" = glm(as.formula(paste(dep_var, "~ Nbr_compte_MS")), data = data_filtered, family = binomial),
    "Modèle 1b" = glm(as.formula(paste(dep_var, "~ Nbr_compte_MS_regulier")), data = data_filtered, family = binomial),
    "Modèle 1c" = glm(as.formula(paste(dep_var, "~ Nbr_compte_MS + Nbr_compte_MS_regulier")), data = data_filtered, family = binomial),
    "Modèle 2"  = glm(as.formula(paste(dep_var, "~ Nbr_compte_MS + Nbr_compte_MS_regulier + age + sexe + Lieu_de_residence")), data = data_filtered, family = binomial),
    "Modèle 3"  = glm(as.formula(paste(dep_var, "~ Nbr_compte_MS + Nbr_compte_MS_regulier + age + sexe + Lieu_de_residence + Diplome_du_repondant")), data = data_filtered, family = binomial)
  )
})
